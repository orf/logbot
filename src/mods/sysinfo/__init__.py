...
class Mod(plugin.Plugin):
    ...
    def display_memory(self, channel, user, arguments):
        ''' Display the total, used and free memory values for the current system '''
        usage = psutil.phymem_usage()
        return self.message(channel, "Total: %s | Used: %s | Free: %s | Percent Free: %s"%(byte_format(usage.total),
                                                                                           byte_format(usage.used),
                                                                                           byte_format(usage.free),
                                                                                           usage.percent))

    def display_disk(self, channel, user, arguments):
        ''' Display the total, used and free disk values for the current system. Does not handle multiple disks.
        '''
        disk = psutil.disk_usage("/")
        return self.message(channel, "Total: %s | Used: %s | Free: %s | Percent Free: %s"%(byte_format(disk.total),
                                                                                           byte_format(disk.used),
                                                                                           byte_format(disk.free),
                                                                                           disk.percent))

    def disk_io_counters(self, channel, user, arguments):
        ''' Display the systems disk IO counters
        '''
        counters = psutil.disk_io_counters()
        return self.message(channel, "Read: %s (%s) | Write: %s (%s)"%(counters.read_count, byte_format(counters.read_bytes),
                                                                       counters.write_count, byte_format(counters.write_bytes)))

    def load_average(self, channel, user, arguments):
        ''' Display the systems load average. Only works on Linux.
        '''
        if not hasattr(os, "getloadavg"):
            return self.message(channel, "Cannot get the system load average, try using cpu average instead")

        return "%.2f %.2f %.2f"%os.getloadavg()

    def cpu_load(self, channel, user, arguments):
        ''' Return the average CPU utilization for all cores measured over 1 second
        '''
        reactor.callInThread(self._cpu_load,channel)

    def _cpu_load(self, channel):
        reactor.callFromThread(self.message, channel, "Measured CPU load over 1 second: %s"%psutil.cpu_percent(interval=1))